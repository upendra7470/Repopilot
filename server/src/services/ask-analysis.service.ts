...
/**
 * Cache key input (Step 14). Must cover every input that can change the
 * answer: repository is a separate column, but question, entity scope,
 * AND the generating provider+model all belong in the hash — otherwise a
 * model switch would serve a stale answer generated by another model.
 */
function hashQuestion(
  question: string,
  context?: { entityType: string; entityId: string },
  aiConfig?: AiConfig | null,
): string {
  const parts = [question];
  if (context) parts.push(`${context.entityType}:${context.entityId}`);
  // "none" keeps key shape stable when no provider is configured.
  parts.push(`${aiConfig?.provider ?? "none"}:${aiConfig?.model ?? "none"}`);
...
function buildInvestigationContextPrompt(investigation: InvestigationContext): string {
  ...
  // Temporal order uses association language only — never causation.
  if (
    temporalRelationships?.changesBefore?.length ||
    temporalRelationships?.changesAfter?.length ||
    temporalRelationships?.incidentTimeline?.length
  ) {
    lines.push("TEMPORAL:");
    temporalRelationships.changesBefore.slice(0, 5).forEach((c) => {
      lines.push(`  ${c.shortSha} occurred before target`);
    });
    temporalRelationships.changesAfter.slice(0, 5).forEach((c) => {
      lines.push(`  ${c.shortSha} occurred after target`);
    });
    temporalRelationships.incidentTimeline.slice(0, 8).forEach((t) => {
      lines.push(`  [${t.kind}] ${t.title}${t.detail ? ` — ${t.detail}` : ""}`);
    });
  }
...
Provide a structured JSON response per the system prompt.`;
  ...
  try {
    ...
    // Single Zod gate: schema-invalid, fabricated, or orphan evidence IDs
    // are rejected here — never passed through on a cast.
    const validIds = new Set(result.evidence.map((e) => e.id));
    const parsed = sanitizePayload(extractJsonObject(raw), validIds);
    if (!parsed) {
      logger.warn("AI response failed schema validation; treating as unavailable");
      return { aiUnavailable: true };
    }
...
export async function requestAskAnalysis(
  ...
  prebuiltDeterministic?: AskResult | null,
): Promise<AskAnalysisResult> {
  const logger = getLogger();
  // Prefer the caller's deterministic package when provided (the ask route
  // already built one) — one question must not trigger two full retrievals.
  const deterministic =
    prebuiltDeterministic ?? (await answerQuestion(repositoryId, question, history, context));
  ...
  const questionHash = hashQuestion(question, context, aiConfig);
